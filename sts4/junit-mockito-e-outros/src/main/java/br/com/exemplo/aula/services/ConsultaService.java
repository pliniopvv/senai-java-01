...
public class ConsultaService {
	...
	public ConsultaService(ConsultaRepository consultaRepository, NutricionistaRepository nutricionistaRepository,
	...
	// Listar consultas apenas com Data/Hora, nome de Nutricionista e nome de
	// Paciente.
	public List<ConsultaResponseListDTO> listarConsultas() {
		return consultaRepository
				.findAll().stream().map(consulta -> new ConsultaResponseListDTO(consulta.getId(),
						consulta.getNutricionista().getNome(), consulta.getPaciente().getNome(), consulta.getData()))
	...
	public ConsultaResponseDTO buscarConsulta(Long id) {
		...
		if (consulta != null) {
			return new ConsultaResponseDTO(consulta.getId(), consulta.getNutricionista(), consulta.getPaciente(),
					consulta.getData(), consulta.getObservacoes());
		}
		return null;
	}
	...
	public ConsultaResponseDTO salvarConsulta(ConsultaRequestDTO request) {
		...
		return new ConsultaResponseDTO(entitySalva.getId(), entitySalva.getNutricionista(), entitySalva.getPaciente(),
				entitySalva.getData(), entitySalva.getObservacoes());
	}
